 ...
 * 底部彈出window
 */
class DialogAgreement(activity: Activity) {
    ...
    private var pwInterface: DialogAgreementInterface? = null
    private var mPwPadding: Float = 10f//Pw默认padding,，默认为10dp
    private var mSelectText: String? = null // 选中的字段



    fun setOnInterface(pwInterface: DialogAgreementInterface): DialogAgreement {
        this.pwInterface = pwInterface
        return this
    }

    fun disMiss(): DialogAgreement {
        mPopupWindow?.dismiss()
        return this
    }


    fun create(): DialogAgreement {
        initView()
    ...
    private fun initView() {
        ...
        mContentView = LayoutInflater.from(mActivity).inflate(R.layout.dialog_agreement, null)
        ...
        with(mContentView) {
            ...
            btAgreement.setOnClickListener {
                mPopupWindow.dismiss()
                pwInterface?.onSuccess()
            }
            tvAgreementViewpage.setMovementMethod(ScrollingMovementMethod.getInstance());
