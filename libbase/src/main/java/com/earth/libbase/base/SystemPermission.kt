...
private const val SYSTEM_FRAGMENT_TAG = "system.permission.fragment.tag"
private const val SYSTEM_REQUEST_CODE = 0xFF
...
private fun FragmentActivity.getPermissionFragment(): KPermissionFragment =
    supportFragmentManager.findFragmentByTag(SYSTEM_FRAGMENT_TAG) as? KPermissionFragment
        ?: KPermissionFragment().apply {
            supportFragmentManager.beginTransaction()
                .add(this, SYSTEM_FRAGMENT_TAG).commitNowAllowingStateLoss()
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        permissions.forEachIndexed { index, permission ->
            if (grantResults[index] == PackageManager.PERMISSION_DENIED) {
                if (shouldShowRequestPermissionRationale(permission)) deniedPermissions.add(
                    permission
                )
...
private object PermissionMap {
    private val atomicInteger = AtomicInteger(SYSTEM_REQUEST_CODE)
