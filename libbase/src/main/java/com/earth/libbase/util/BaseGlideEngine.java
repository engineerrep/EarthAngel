...
public class BaseGlideEngine implements ImageEngine {
    //单例
    private static BaseGlideEngine instance = null;
    //单例模式，私有构造方法
    private BaseGlideEngine() {
    }
    //获取单例
    public static BaseGlideEngine getInstance() {
        if (null == instance) {
            synchronized (BaseGlideEngine.class) {
                if (null == instance) {
                    instance = new BaseGlideEngine();
                }
            }
        }
        return instance;
